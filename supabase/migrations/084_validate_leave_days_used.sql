-- 084_validate_leave_days_used.sql
--
-- validate_and_submit_leave에 p_days_used 서버측 검증 추가
-- 클라이언트가 조작된 days_used를 보내도 서버에서 차단
...
BEGIN
  ...
  -- p_days_used 서버측 범위 검증 (클라이언트 조작 방어)
  IF p_leave_type IN ('AM_HALF', 'PM_HALF', 'HALF_DAY') THEN
    IF p_days_used != 0.5 THEN
      RAISE EXCEPTION '반차 차감일수는 0.5일이어야 합니다.';
    END IF;
  ELSIF p_leave_type IN ('ANNUAL', 'GROUP') THEN
    v_max_days := (p_end_date - p_start_date + 1)::NUMERIC;
    IF p_days_used <= 0 OR p_days_used > v_max_days THEN
      RAISE EXCEPTION '차감일수(%)가 유효 범위(0 < days <= %)를 벗어났습니다.', p_days_used, v_max_days;
    END IF;
  END IF;
