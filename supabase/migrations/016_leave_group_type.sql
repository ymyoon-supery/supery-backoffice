-- 공동연차(GROUP) 유형 추가 + validate_and_submit_leave 차감 대상 업데이트
...
ALTER TABLE leave_requests ADD CONSTRAINT leave_requests_leave_type_check
  CHECK (leave_type IN ('ANNUAL', 'SICK', 'HALF_DAY', 'COMP', 'GROUP', 'OTHER'));
...
BEGIN
  ...
  -- ANNUAL, HALF_DAY, GROUP 은 잔여 연차에서 차감
  IF p_leave_type IN ('ANNUAL', 'HALF_DAY', 'GROUP') THEN
