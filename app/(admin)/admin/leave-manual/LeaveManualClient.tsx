            ...
            {recordYears.length > 0 && (
              <div className="flex items-center gap-1">
                <button
                  ...
                  className={`px-2.5 py-1 text-xs rounded-lg border transition-colors ${!filterYear ? 'bg-gray-700 text-white border-gray-700' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`}
                >전체</button>
                {recordYears.map(y => (
                  <button key={y} onClick={() => setFilterYear(y)}
                    className={`px-2.5 py-1 text-xs rounded-lg border transition-colors ${filterYear === y ? 'bg-primary text-white border-primary' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`}
                  >{y}년</button>
                ))}
              </div>
            )}
